...
func (t *Task) Validate() error {
	...
	if t.Completed {
		if t.CompletedAt == nil {
			return fmt.Errorf(
				"`CompletedAt` cant be null if `Completed`==`true`: %w",
				core_errors.ErrInvalidArgument,
			)
		}
...
func NewPatchTask(title Nullable[string], description Nullable[string], completed Nullable[bool]) TaskPatch {
	return TaskPatch{
		Title: title,
		Description: description,
		Completed: completed,
...
func (p *TaskPatch) Validate() error {
	if p.Title.Set && p.Title.Value == nil {
		return fmt.Errorf("`Title` cant be patched to NULL")
	}
	if p.Completed.Set && p.Completed.Value == nil {
		return fmt.Errorf("`Completed` cant be patched to NULL")
	}
	return nil
}

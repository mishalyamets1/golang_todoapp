...
func (r *TasksRepository) PatchTask(ctx context.Context, id int, task domain.Task) (domain.Task, error) {
	...
	query := `UPDATE todoapp.tasks SET title=$1, description=$2, completed=$3, completed_at=$4, version=version+1 WHERE id=$5 AND version=$6 RETURNING *;`
	...
	if err != nil {
		if errors.Is(err, core_postgres_pool.ErrNoRows) {
			return domain.Task{}, fmt.Errorf("task with id='%d' concurrenlt accessed: %w", id, core_errors.ErrConfilct)
		}
